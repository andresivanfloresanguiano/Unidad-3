            ...
            for linea in archivo:
                if linea.strip():
                    ...
                    if len(datos) == 4:
                        ...
                        total = float(total)
                        total_ventas += total  # Acumulando el total
                        tabla.insert("", tk.END, values=(producto, precio, cantidad, total))
    except FileNotFoundError:
        messagebox.showerror("Error", "El archivo ventas.txt no existe.")
        ventana.destroy()
        return

    # Mostrar el total de ventas al final, debajo de la tabla
    lbl_total_ventas = tk.Label(ventana, text=f"Total Ventas: ${total_ventas:.2f}", font=("Arial", 12, "bold"), bg="#f2f2f2")
    lbl_total_ventas.pack(pady=10)
...
estilo.configure("Negro.TButton", font=("Arial", 12), foreground="white", background="black", padding=10)
estilo.map("Negro.TButton", background=[("active", "#1a1a1a")], foreground=[("active", "white")])
